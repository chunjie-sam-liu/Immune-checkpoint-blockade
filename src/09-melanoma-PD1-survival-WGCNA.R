#WGCNA3.r expr_R.txt 0.90 30 0.25
#WGCNA3.r expr_NR.txt 0.90 30 0.25
...
#NR_black NR_ivory R_white plot---------------------------------------------------------------------
Combined_data %>%
  dplyr::select(Run,Survival_time,Survival_status,NR_black)->Combined_data_plot

cutoff=mean(Combined_data_plot$NR_black)
Combined_data_plot %>%
  dplyr::mutate(Class=if_else(NR_black <= cutoff , "lower","higher"))->Combined_data_plot
fit <- survfit(Surv(Survival_time, Survival_status) ~ Class, data = Combined_data_plot)

pdf(file="/data/liull/immune-checkpoint-blockade/coexpress_modules/test_WGCNA_R/SRP094781/NR_black_survival.pdf")
