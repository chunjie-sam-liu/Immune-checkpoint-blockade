...
read.table("/data/liull/test_coXpress/all_groups_R.txt",header = T,as.is = TRUE)%>%
  dplyr::filter(N>8)->all_groups
all_groups%>%
  dplyr::filter(pr.g1<=0.05)%>%
  dplyr::filter(pr.g2>=0.05)->selected_group_R#7
all_groups%>%
  dplyr::filter(pr.g1>0.05)%>%
  dplyr::filter(pr.g2<0.05)->selected_group_NR#0
rbind(selected_group_R,selected_group_NR)->selected_groups

read.table("/data/liull/test_coXpress/g_R.txt",header = F,as.is = TRUE) ->g_dataframe
g=g_dataframe$V2
