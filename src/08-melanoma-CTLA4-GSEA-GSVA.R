...
all_expression %>%
  dplyr::select(gene_id,response_ids,non_response_ids)->melanoma_expr
row.names(melanoma_expr)=melanoma_expr[,1]
melanoma_expr=melanoma_expr[,-1]

DGEList_expr <- DGEList(counts=melanoma_expr)
...
normalized_loggedCPM_expr <- normalized_loggedCPM_expr[keep,]#the same normalized(log2CPM) as melanoma PD1
dim(normalized_loggedCPM_expr)
